...
router.get('/visuel', function(req, res) {
	...
	Visuel.find(function(err, results) {
		...
		res.render('back/adminVisuel.hbs', { visuels: results, title: 'Visuel du site' });
	});
});
	...
	Visuel.update(
		{
			...
			transition1: req.body.transition1,
			transition2: req.body.transition2,
			transition3: req.body.transition3
		},
		function(err, numRowsAffected, rawResponse) {
			if (err) throw err;
			res.redirect('back/adminVisuel');
		}
	);
});
...
router.get('/showroom', async (req, res) => {
	try {
		const [ logos, audreacts, bumpers, vjings ] = await Promise.all([
			Logo.find(),
			Audreact.find(),
			Bumpers.find(),
			Vjing.find()
